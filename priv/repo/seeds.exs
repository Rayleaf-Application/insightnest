...
if Repo.aggregate(Member, :count) > 0 do
  ...
  slow_knowledge =
    Repo.get_by!(Insightnest.Sparks.Spark, slug: "the-case-for-slow-knowledge-a1b2c3")
  ...
  if Repo.aggregate(Contribution, :count) == 0 do
    contributions = [
      %{
        ...
        author_id: bob.id,
        body:
          "There's strong empirical grounding here. Studies on deliberative democracy show that slowing down information processing significantly improves the quality of conclusions drawn. The Kahneman System 1/2 framing applies directly — fast knowledge is System 1, pattern-matched and often wrong.",
        stance: "evidence",
        inserted_at: DateTime.add(now, 3600, :second),
        updated_at: DateTime.add(now, 3600, :second)
      },
      %{
        ...
        author_id: carol.id,
        body:
          "I'd push back on the implicit assumption that speed is purely a platform incentive problem. Sometimes urgency is epistemically valid — breaking news, emergency coordination, time-sensitive decisions. The question isn't slow vs. fast but appropriate speed for context.",
        stance: "challenges",
        inserted_at: DateTime.add(now, 7200, :second),
        updated_at: DateTime.add(now, 7200, :second)
      },
      %{
        ...
        author_id: alice.id,
        body:
          "This connects directly to Yochai Benkler's work on commons-based peer production. The Wealth of Networks argues that non-rival goods enable new production models that weren't economically viable before. InsightNest is essentially testing whether knowledge crystallisation is one of them.",
        stance: "expands",
      ...
      %{
        ...
        author_id: carol.id,
        body:
          "What's the governance model when contributors disagree about what counts as a valid contribution? Wikipedia's edit wars suggest this is harder than it looks at the protocol level.",
        stance: "question",
